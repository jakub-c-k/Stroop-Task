...
wordList = {'Red', 'Green', 'Blue', 'Brown', 'Pink', 'White', 'Purple', 'Yellow'};
...
condMatrixSkewed = [];
trialsCongruent_skewed = round((128 / 100) * 80); % 40% congruent trials
trialsIncongruent_skewed = round((128 / 100) * 20); % 20% incongruent trials
...
condMatrixIncongruent_skewed = condMatrixIncongruent_skewed(:, 1:trialsIncongruent_skewed); 

% Combine the selected congruent and incongruent conditions
condMatrixSkewed = [condMatrixCongruent_skewed, condMatrixIncongruent_skewed];3

condMatrixSkewed(3,:) = repmat(cues, 1, ceil(numTrials / numCues)); % Ensure even distribution of cue types
condMatrixSkewed(3,:) = condMatrixSkewed(3, 1:numTrials);
% Randomise the conditions
shuffler = Shuffle(1:numTrials);

condMatrixShuffled = condMatrixSkewed(:, shuffler);
...
for trial = 1:numTrials
    ...
    if trial == 1
        ...
        DrawFormattedText(window, 'When you are ready, press any button to begin',...
            'center', 'center', black);
    ...
    if trial == numTrials/2
        ...
        DrawFormattedText(window, 'You are a 1/4 through the task. Please take a break. \n\n \n\n When you are ready, press any button to continue',...
            'center', 'center', black);
